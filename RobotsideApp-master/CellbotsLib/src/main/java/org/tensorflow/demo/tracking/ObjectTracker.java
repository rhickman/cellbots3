...
class ObjectTracker {
  ...
  static {
    try {
      ...
      libraryFound = true;
        LOGGER.e("libtensorflow_demo.so found, tracking available!");
  ...
  private static class TimestampedDeltas {
    ...
    private TimestampedDeltas(final long timestamp, final byte[] deltas) {
  ...
  public static class FrameChange {
    ...
    public FrameChange(final float[] framePoints) {
      ...
      pointDeltas = new Vector<>(framePoints.length / KEYPOINT_STEP);
  ...
  public static synchronized ObjectTracker getInstance(
    ...
    if (instance == null) {
      ...
      throw new RuntimeException(
          "Tried to create a new object tracker before releasing the old one!");
    }
    return instance;
  }
  ...
  protected ObjectTracker(
    ...
    this.debugHistory = new Vector<>(MAX_DEBUG_HISTORY_SIZE);
    this.downsampledFrame =
        new byte
  ...
  public synchronized List<byte[]> pollAccumulatedFlowData(final long endFrameTime) {
    final List<byte[]> frameDeltas = new ArrayList<>();
