 ...
 */
public final class TangoInitializationHelper {
    // We use this custom tag instead of class simple name because the maximum tag length for logging
    // is 23.
    ...
    public static final boolean bindTangoService(final Context context,
        ...
        // User doesn't have a Java TangoCore at all; fall back to the deprecated approach
    ...
    public static final int loadTangoSharedLibrary() {
        ...
        try {
            ...
            Log.i(TAG, "Success! Using arm64-v8a/libtango_client_api.");
        ...
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i(TAG, "Success! Using armeabi-v7a/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i(TAG, "Success! Using x86_64/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                loadedSoId = ARCH_X86;
                Log.i(TAG, "Success! Using x86/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i(TAG, "Success! Using default/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i(TAG, "Falling back to libtango_client_api.so symlink.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        return loadedSoId;
